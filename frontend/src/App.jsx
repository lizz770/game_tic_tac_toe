...
// предотвратите повторный рендеринг строки tictactoe и записей с одинаковым значением
...
const getStatusMsg = ({
  ...
  if (finished) {
    ...
    return 'Ждем, когда к нам присоединится другой игрок...';
  } if (status === 'inGame') {
    if (plrToMove.id === curPlr?.id) {
      return 'Теперь твоя очередь; сделай ход';
    }
    return `Ждем, когда другой игрок ${plrToMove.username} сделает свой ход...`;
  }
  return 'Ошибка: Вы никогда не должны этого видеть. Свяжитесь с разработчиками!';
};
